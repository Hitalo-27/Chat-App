...
export default function CreateGroup() {
   ...
   const [imageUri, setImageUri] = useState(`http://192.168.178.66:8080/${user ? user.imageName : ''}`);
   ...
   const handleCreateGroup = async () => {
      try {
         ...
         const responseCreate = await axios.post(
            `http://192.168.178.66:8080/group/create`,
            formData,
            {
               headers: {
         ...
         await Promise.all(selectedUsers.map(async (selectedUser) => {
            const responseAddUser = await axios.post(
               `http://192.168.178.66:8080/group/add/${responseCreate.data.message.conversationId.id}`,
               {
